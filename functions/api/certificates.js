...
export async function onRequest(context) {
  ...
  if (method === 'GET') {
    const { results } = await env.PORTFOLIO_DB.prepare('SELECT * FROM certificates ORDER BY sort_order ASC, id ASC').all();
    return json(results);
  }
  ...
  if (method === 'PUT' || method === 'PATCH' || method === 'DELETE') {
    ...
    if (!id) return json({ message: 'Missing certificate id' }, 400);

    if (method === 'DELETE') {
      await env.PORTFOLIO_DB.prepare('DELETE FROM certificates WHERE id = ?').bind(id).run();
      return json({ ok: true });
    }
